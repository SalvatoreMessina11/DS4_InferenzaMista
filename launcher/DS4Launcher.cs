...
class DS4Launcher : Form {
    ...
    void Summary(){int n=Count();summary.Text=Solo?"Chat locale: tutto il modello su questo PC con CUDA e streaming.\r\nNon richiede un secondo PC, IP LAN o regole firewall.":role.SelectedIndex==0?"Qui scrivi i prompt. Livelli locali: 0-"+(n-1)+".\r\nSull'altro PC scegli Worker; attendi che si colleghi.":"Qui non si inviano prompt. Livelli locali: "+n+"-42 e output.\r\nSull'altro PC scegli Coordinatore.";local.Enabled=peer.Enabled=firewall.Enabled=!busy&&!Solo;gpu.Enabled=!busy&&!Solo;}
    void ValidateFields(){if(!Solo){local.Text=IPv4(local.Text.Trim());peer.Text=IPv4(peer.Text.Trim());if(local.Text==peer.Text)throw new Exception("I due PC devono avere indirizzi diversi.");}if(!Regex.IsMatch(distro.Text,@"^[A-Za-z0-9_.-]+$") || (user.Text.Length>0 && !Regex.IsMatch(user.Text,@"^[a-z_][a-z0-9_-]*$")))throw new Exception("Nome distribuzione o utente Ubuntu non valido.");if(folder.Text.Length>0 && (!folder.Text.StartsWith("/") || folder.Text.Contains("\n") || folder.Text.Contains("\r")))throw new Exception("Usa un percorso Linux assoluto, ad esempio /home/nome/DS4_InferenzaMista.");}
...
(launching ? "if pgrep -x ds4 >/dev/null || pgrep -x ds4-server >/dev/null; then echo 'ERRORE: DS4 gia attivo. Chiudi prima la sessione precedente.'; exit 1; fi\n"+(Solo?"":"if [ -n \"$(ss -ltnH 'sport = :"+(role.SelectedIndex==0?"9911":"9912")+"')\" ]; then echo 'ERRORE: porta occupata, forse da un listener diagnostico. Fermalo prima di avviare.'; exit 1; fi\n") : "")+"echo VERIFICA_OK\n";}
    ...
    string ModelCommand(){return Solo?"./ds4 --cuda --ssd-streaming --ctx 2048 --prefill-chunk 64 --nothink -n 256":Command(role.SelectedIndex==0,Count(),role.SelectedIndex==0?local.Text:peer.Text);}
    string MemoryEnvironment(){return "export DS4_CUDA_NO_DIRECT_IO=1\n"+(keepRam.Checked?"export DS4_CUDA_KEEP_MODEL_PAGES=1\n":"unset DS4_CUDA_KEEP_MODEL_PAGES\n");}
