...
Il launcher include **Chat DS4 / Pi Agent**, **Wi-Fi/LAN / Ethernet diretto**, icona dedicata, context **100K / 300K / personalizzato** (2048–1000000), split configurabile e Stop. Default context 100000: e una richiesta al motore, non una garanzia che il contesto entri nella memoria disponibile. Pi richiede un'installazione separata in Ubuntu e usa solo l'API locale. [Comandi, prove e limiti della nuova versione](launcher/VALIDATION.md).

Nella Chat DS4, prefill e generazione aggiornano il **titolo della console** ogni mezzo secondo con token, token/s medi e tempo. Nel prefill distribuito i token confermati avanzano per blocchi: il timer continua durante l'attesa. Il testo della risposta resta separato dalle metriche.
