...
(launching ? "if pgrep -x 'ds4|ds4-server|llama-cli|llama-server|ggml-rpc-server' >/dev/null; then echo 'ERRORE: DS4 gia attivo. Chiudi prima la sessione precedente.'; exit 1; fi\n"+(Solo?"":"if [ -n \"$(ss -ltnH 'sport = :"+(role.SelectedIndex==0?"9911":"9912")+"')\" ]; then echo 'ERRORE: porta occupata, forse da un listener diagnostico. Fermalo prima di avviare.'; exit 1; fi\n")+(PiMode?"if [ -n \"$(ss -ltnH 'sport = :8000')\" ]; then echo 'ERRORE: porta HTTP 8000 gia occupata'; exit 1; fi\n":"") : "")+"echo VERIFICA_OK\n";}
    ...
    void LoadConfig(){string source=File.Exists(Config)?Config:Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"DS4Launcher","settings.xml");if(!File.Exists(source))return;try{var saved=XDocument.Load(source).Root;ApplySettings(saved);if((int?)(saved.Element("advanced")??new XElement("advanced")).Element("defaultsVersion")!=2){role.SelectedIndex=2;model.SelectedIndex=1;frontend.SelectedIndex=1;maxOutput.Value=32768;}}catch{}}
